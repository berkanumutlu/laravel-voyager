...
class TicketController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $search = (object) [
            'value' => $request->get('s'), 'key' => $request->get('key'), 'filter' => $request->get('filter')
        ];
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model,
                    'scope'.ucfirst($dataType->scope))) {
                $query->{$dataType->scope}();
            }

            // Use withTrashed() if model uses SoftDeletes and if toggle is selected
            if ($model && in_array(SoftDeletes::class, class_uses_recursive($model)) && Auth::user()->can('delete',
            ...
            if ($search->value != '' && $search->key && $search->filter) {
                ...
                if ($row = $this->findSearchableRelationshipRow($dataType->rows->where('type', 'relationship'),
                    ...
                    $query->whereIn(
                        $searchField,
                        $row->details->model::where($row->details->label, $search_filter,
                            $search_value)->pluck('id')->toArray()
                    );
                } else {
    ...
    protected function cleanup($dataType, $data)
    {
        ...
        $dataType->rows->where('type', 'media_picker')->where('details.delete_files', true)->each(function ($row) use (
            $data
        ) {
    ...
    public function relation(Request $request)
    {
        ...
        foreach ($rows as $key => $row) {
            if ($row->field === $request->input('type')) {
                ...
                if (isset($options->scope) && $options->scope != '' && method_exists($model,
    ...
    public function reply(Request $request, $id)
    {
        ...
        $ticket_message->ticket_id = $id;
        $ticket_message->sender_id = auth()->user()->id;
